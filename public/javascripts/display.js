...
DisplayBillHTML = function(relation,bill){
    ...
    HTML+=toDateString(bill.date)+"  "+ bill.descript +" : "+bill.buyer.pseudo+" a payé "+ bill.buyer.cost+ "€";
...
DisplayPaymentHTML= function(relation,payment){
    ...
    HTML+=toDateString(payment.date)+" Remboursement: "+payment.giver.pseudo+" a donné "+payment.cost+"€ à "+payment.reciever.pseudo;
    if(relation.type =='GROUP'){
    	HTML +=" dans le groupe \""+relation.name+"\"";
    }
